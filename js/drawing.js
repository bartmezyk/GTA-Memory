const drawing = () =>
{
	...
	if (sequence.length==0) drawedNumber = Math.floor(Math.random() * 4); //Gdy pierwszy raz włączamy grę następuje losowanie z wszystkich 4 postaci.
	else //W przeciwnym wypadku poprzednio wylosowana postać ma nie być brana pod uwagę przy kolejnym losowaniu.
	{
		...
		for(i = 0; i < 4; i++)
		{
			if(i != sequence[sequence.length - 1]) ///Tylko numer postaci poprzednio wylosowanej nie znajdzie sie w tymczasowej tablicy numerów postaci do kolejnego losowania.
			{
	...
	sequence.push(drawedNumber); //Umieszczenie wylosowanego nr postaci w globalnej tablicy liczb będących sekwencją numerów postaci do kliknięcia.
